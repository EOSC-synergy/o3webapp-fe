...
class GenerationPage extends React.Component {
  ...
  fetchModelsFromApi() {
    ...
    const request_url =
      BACKEND_SERVER_URL + configData.MODEL_LIST_PATH + "/" + currplotType;
  ...
  handlePlotTypeChange(plotType) {
    ...
    //logic to disable currently not enabled plot types
    //TODO remove when all types are available
    if (plotType === "tco3_return" || plotType === "vmro3_zm") {
      alert("This plot type is currently not available!");
  ...
  handleLatitudeChange(latitude_array) {
    ...
    var lat_min =
      latitude_array[0] !== undefined
        ? latitude_array[0]
        : configData.GENERATION_DEFAULTS.LAT_MIN;
    var lat_max =
      latitude_array[1] !== undefined
        ? latitude_array[1]
        : configData.GENERATION_DEFAULTS.LAT_MAX;
      ...
      <div className="generation-container">
        <div className="generation-form-wrapper">
          <form onSubmit={this.handleSubmit} className="generation-form">
            <PlotButtonController handleChange={this.handlePlotTypeChange} active={pType}/>
            ...
            <div className="settings-section-wrapper section-wrapper">
              <div className="year-section-wrapper section-wrapper">
                {availableSettings.some(
                  (setting) =>
                    setting.name === "begin" || setting.name === "end"
                ) && (
                  <div>
              ...
              {availableSettings.some(
                (setting) =>
                  setting.name === "lat_min" || setting.name === "lat_max"
              ) && (
                <LatitudeButton
            ...
            <input
              ...
              value="Submit"
              id="submitButton"
              className="submit-button mat-style-accent"
            />
          </form>
        </div>
      </div>
